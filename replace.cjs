...
<body>
  ...
  <div id="controls">
    ...
    <div class="control-group">
      <label><span>Rotation Speed</span> <span id="vRot">20</span></label>
      <input type="range" id="rotSpeed" min="0" max="100" value="20">
    </div>
    <div class="control-group">
      <label><span>Camera Distance</span> <span id="vDist">80</span></label>
      <input type="range" id="camDist" min="30" max="200" value="80">
    </div>
  </div>
  <script>
    ...
    const elDist = document.getElementById('camDist');
    ...
    const updateLabels = () => {
        ...
        vDist.innerText = elDist.value;
    };
    [elAmp, elTwist, elGlow, elRot, elDist].forEach(el => el.addEventListener('input', updateLabels));
    ...
    function noise(x, y, z) {
        let p = new THREE.Vector3(Math.floor(x), Math.floor(y), Math.floor(z));
        let f = new THREE.Vector3(x-p.x, y-p.y, z-p.z);
    ...
    function snoiseVec3(x, y, z) {
        return new THREE.Vector3(
            noise(x, y, z),
            noise(x+13.5, y+23.1, z+11.3),
            noise(x+41.2, y+17.8, z+37.5)
        );
    }
    
    function curlNoise(x, y, z, scale) {
        let e = 0.1;
        let p = new THREE.Vector3(x,y,z).multiplyScalar(scale);
        ...
        return new THREE.Vector3(cx, cy, cz).divideScalar(2*e);
    }
    ...
    function animate() {
        ...
        let camZ = parseFloat(elDist.value);
        
        camera.position.lerp(new THREE.Vector3(0, 0, camZ), 0.05);
        group.rotation.y += rot * 0.02;
  ...
  <\\/script>
    ...
    const updatedCode = code.substring(0, startIdx) + newAlgo + code.substring(nextIdx);
