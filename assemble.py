...
top = full_content.split("  metadata: { label: string; value: string }[  ,{")[0]
if not top.endswith("\n"): top += "\n"
top += "  metadata: { label: string; value: string }[];\n};\n\nconst artworks: Artwork[] = [\n"

# `artworks_good.txt` has the contents from line 16 to 1048.
# Line 16 is `  metadata: { label: string; value: string }[  ,{`
# But wait! There's no need to use `artworks_good.txt` if I can just slice the string!
start_algo1 = full_content.find("  {\n    id: \"algo-0x01\",")
if start_algo1 == -1:
    start_algo1 = full_content.find("  {\n    id: 'algo-0x01',")
if start_algo1 == -1:
    # Just find algo-0x01
    start_algo1 = full_content.find("    id: \"algo-0x01\",") - 4

# End of good parts is right before `algo-0x0c` injection which ruined algo-0x0b.
# But wait! Let's just find `function resize() {` inside `algo-0x0b`.
idx_0xb = full_content.find("id: \"algo-0x0b\"")
idx_resize = full_content.find("function resize() {", idx_0xb)
idx_resize_end = full_content.find("    }", idx_resize) + 5

# Get the good artworks part
artworks_good = full_content[start_algo1:idx_resize_end]
...
idx_react = full_content.find("\nexport default function App() {")
react_part = full_content[idx_react:]

new_file = top + artworks_good + algo_0xb_tail + "\n" + algo_0xc + react_part
