...
if (startIdx !== -1 && nextIdx !== -1) {
    ...
    // Completely replace the htmlContent to ensure it's clean and functional
    const htmlContentStart = segment.indexOf('htmlContent: `') + 14;
    const htmlContentEnd = segment.indexOf('`,\n    metadata:');
...
<body>
  ...
  <\\/script>
    ...
    code = code.substring(0, htmlContentStart) + newHtmlContent + code.substring(htmlContentEnd);
